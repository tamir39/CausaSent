...
[**🤗 Dataset**](https://huggingface.co/datasets/Tamir39/causasent-ate-v2) · [**🤗 Model**](https://huggingface.co/Tamir39/causasent-phobert-ate)
...
The model is deliberately tuned recall-first on ATE; the aggregation step absorbs span-level false positives via frequency ranking — span sai (false positives) chỉ xuất hiện 1-2 lần và bị drown khi top-K ranking, trong khi aspect category và sentiment label (hai thứ phải đúng) đều là chỗ model làm tốt nhất.
...
A 300-record stratified validation pass by an independent Claude judge agreed with the originally-extracted span in **273 / 300 cases (91.00%)**.
...
scripts/         CLI utilities + run.bat helpers + docx report generator
...
(`Tamir39/causasent-ate-v2`) and is not bundled with the source tree —
fetch it once locally with `huggingface_hub.snapshot_download`. The
trained checkpoint is on `Tamir39/causasent-phobert-ate`, pulled by
`run.bat`'s quick-start above.
